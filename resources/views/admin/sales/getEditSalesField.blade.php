    ...
    <div class="row" id="{{ $id }}">
                ...
                <select class="form-control" name="product_name[]" onchange="changeOncePrice(this, {{ $id }})">
                    @foreach ($products as $product)
                        <option value="{{ $product->name }}" product-price="{{ $product->price }}"
                            @if ($product->name == $product_name[$i]) selected @endif>
                            {{ $product->name }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="col-md-3">
            <div class="form-group">
                ...
                <input type="text" value="{{ $quantity[$i] ?? old('quantity') }}"
                    class="quantity-{{ $id }} form-control" placeholder="@lang('translate.quantity_placeholder')"
                    name="quantity[]" maxlength="4" onkeyup="getTotalPrice({{ $id }})">
        ...
        <div class="col-md-3">
            <div class="form-group">
                ...
                <input type="text" value="{{ $once_price[$i] ?? $products[0]->price }}"
                    class="once-price-{{ $id }} form-control" name="once_price[]" readonly>
                @error('once_price')
                    <span class="text-danger">@lang('translate.'.$message)</span>
                @enderror
            </div>
